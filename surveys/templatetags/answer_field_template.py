...
def draw_answer(parser, token):
    try:
        tag_name, question_id, question_type, answer_id, answer_text = token.split_contents()
    except ValueError:
        raise template.TemplateSyntaxError("Не переданы необходимые параметры: question_type, answer_id, answer_text")
    return AnswerNodeObject(question_id, question_type, answer_id, answer_text)


class AnswerNodeObject(template.Node):
    """Шаблон отображения ответов в опросе"""
    def __init__(self, question_id, question_type, answer_id, answer_text):
        self.question_id = template.Variable(question_id)
    ...
    def get_answer_field_template(self, question_id, question_type, answer_id, answer_text):
        template = {
            2: f"""
                <div class="custom-control custom-radio">
                    <input type="radio" id="{answer_id}" name="answer-{question_id}" class="custom-control-input">
    ...
    def render(self, context):
        resolve_question_id = self.question_id.resolve(context)
